...
- `FirebaseCallableCareerStateSyncService` calls `syncOpenLARPCareerState` to reconcile a bounded, versioned Rich V0 snapshot under `users/{uid}/careerState/current`. The callable owns revisions, returns conflicts instead of silently overwriting, and is not yet deployed to the dev project.
...
Firestore rules enforce owner/path consistency, block client-written external action claims and sync status fields, block embedded proof attachment arrays, and restrict client proof attachment metadata to `pendingUpload`. Private goal context, private outcomes, proof records, and proof attachment metadata require an accepted `users/{uid}/consents/privateEvidenceCloudSync` document; `shareWins` alone does not satisfy this gate. Client writes are denied for consent documents and versioned career-state snapshots. Consent recording and revocation are server-owned through `setPrivateEvidenceCloudSyncConsent`; snapshot revision/conflict arbitration is server-owned through `syncOpenLARPCareerState`; proof receipt promotion, backup cleanup, event acknowledgement, and account deletion remain server-owned through their callables. The callable backend records per-user daily quota units before AI workflow dispatch, career-state sync, proof receipt promotion, proof upload reconciliation, uploaded proof backup cleanup, or backend event acknowledgement. It also reserves and reconciles estimated provider spend in the server-only daily AI budget ledger before private AI dispatch. Account deletion is intentionally not quota-gated. This remains a bounded beta snapshot plus career-graph model, not authority for derived readiness claims; production trust still requires signed-in account-control/sync smoke testing, final support/legal copy, derived server writes, App Check enforcement after provider rollout, and signed-in simulator/device checks.
...
- `runOpenLARPWorkflow`, `syncOpenLARPCareerState`, `promoteProofUploadReceipt`, `reconcileProofUploads`, `cleanupRevokedPrivateEvidenceUploads`, and `acknowledgeBackendEvents` are protected by server-side per-user daily callable quota units. Exhausted non-AI calls return `resource-exhausted` before side effects; AI quota exhaustion returns a successful deterministic fallback with `fallbackReason: quota` so the primary product flow remains usable.
