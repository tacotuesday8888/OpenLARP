...
Proof review uses a deliberately split authority model. The model's output schema contains only a bounded coaching reason and actionable improvement. The server attaches deterministic acceptance, score, label, rewards, and inspection scope; linked destinations and attachment contents are always marked uninspected. The iOS client independently recomputes those same authority fields, rejects unsupported inspection language, and uses only safe coaching text from the live response. A model response cannot upgrade an uninspected link or image into verified evidence. Provider failure keeps the draft intact and uses the honest local fallback.

Claiming a proof or self-report creates an editable evidence card beside the immutable receipt. The card records the completed action, quest, readiness gap, proof type, source, confirmation state, timestamp, provenance, user note, private note, and potential future use. Evidence-card private text follows the same explicit private-evidence cloud-sync consent as proof bodies and attachments.

The callable exports are `runOpenLARPWorkflow`, `syncOpenLARPCareerState`, `setPrivateEvidenceCloudSyncConsent`, `promoteProofUploadReceipt`, `reconcileProofUploads`, `cleanupRevokedPrivateEvidenceUploads`, `acknowledgeBackendEvents`, and `deleteOpenLARPAccount`, configured in `firebase.json` under `backend/functions`. The existing dev deployment does not yet include the new career-state callable and still uses deterministic AI fallback; these updated callable/private-service changes have not yet been deployed or live-smoked.

The deterministic callable package includes server-side per-user daily quota units for AI, career-state snapshot, proof, retention, and event callables. `runOpenLARPWorkflow` is capped before deterministic AI dispatch, records provider token/cost estimate metadata without prompt or proof text, and state/proof/event callables are capped before Storage or Firestore side effects. Exhausted users receive `resource-exhausted` with safe quota details only.
