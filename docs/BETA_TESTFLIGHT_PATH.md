...
- The workflow runs Apple's accessibility audit across the core onboarding-to-first-proof path. All reported issues remain failures except Xcode 26.6's Dynamic Type false positive for the identified native SwiftUI onboarding-choice label; that one label is separately exercised at the largest accessibility text size with one-column layout, minimum hit-height, and selection assertions. Manual VoiceOver and supported-device layout checks remain required.
