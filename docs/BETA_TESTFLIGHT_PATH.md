...
- RevenueCat iOS SDK `5.79.0` is linked through XcodeGen/SPM behind `OpenLARPSubscriptionServicing`, with ignored local plist configuration, offering/package purchase boundaries, and a no-key fallback that keeps local beta mode working.
...
- Subscription refresh, offering load, package purchase, restore, paywall exposure, one-time free sprint measurement, and RevenueCat customer-info mapping are wired through the store boundary.
...
5. Create RevenueCat/App Store products, add ignored local `RevenueCat-Info.plist`, replace the compact purchase CTA with the designer-led paywall, and complete sandbox/Test Store purchase verification.
