...
- The workflow runs Apple's accessibility audit across the core onboarding-to-first-proof path. All reported issues remain failures except two narrowly reproduced Xcode 26.6 false positives: the identified native SwiftUI onboarding-choice label is separately exercised at the largest accessibility text size with one-column layout, minimum hit-height, and selection assertions, while the exact disabled onboarding primary-action label is accepted only when its element type, label, disabled state, containing button, and frame match and the shared color-token contract remains above 7:1 contrast. Manual VoiceOver and supported-device layout checks remain required.
