...
- The iOS workflow includes an exact fresh-user UI journey for the public local-only build, covering onboarding, diagnostic review, mission approval, all 14 persisted daily proof loops, the Day 7 report and adaptive second chapter, the Day 14 report, and starting another sprint before the optimized Release contract runs.
- The workflow runs Apple's accessibility audit across the core onboarding-to-first-proof path. All reported issues remain failures except three narrowly reproduced Xcode 26.6 false positives: the identified native SwiftUI onboarding-choice label is separately exercised at the largest accessibility text size with one-column layout, minimum hit-height, and selection assertions; the exact disabled onboarding primary-action label is accepted only when its element type, label, disabled state, containing button, and frame match and the shared color-token contract remains above 7:1 contrast; and the target-outcome element-detection report is accepted only when Xcode supplies no element and explicit UI assertions verify the screen's text, choices, field label, and disabled action in the accessibility hierarchy. Manual VoiceOver and supported-device layout checks remain required.
- CI compiles both the public local-only and internal service-enabled targets as unsigned generic Release builds, in addition to exercising the internal target's Debug test suite on a simulator.
...
- AI workflow callables now record backend-only provider token/cost estimates without prompt or proof text, and live-model mode requires explicit provider pricing plus daily budget config before dispatch.
- Signed-in Firebase CLI smoke tooling exists to verify workflow callable auth, career-state upload/restore/conflict resolution, proof Storage object verification, proof receipt promotion, proof upload reconciliation, backend event acknowledgement, and cleanup of temporary smoke data in the dev project.
...
- `npm run firebase:signed-in-smoke` passed against the Firebase dev project on June 20, 2026, covering the then-deployed temporary-session, deterministic workflow, private-evidence, proof, event, deletion, and cleanup paths. The current smoke additionally requires career-state upload/restore/conflict resolution, so a fresh complete pass remains pending until `syncOpenLARPCareerState` is deployed. The smoke fails early if Firestore or Storage App Check enforcement is already enabled without a supported smoke App Check token path.
...
2. Keep `npm run firebase:signed-in-smoke` passing before each backend-readiness pass, then test account-backed proof attachment uploads, Firestore career graph writes, and the signed-in deterministic callable AI fallback route on a simulator/device.
...
`npm run beta:gate` checks repo-controlled release gates such as the privacy manifest, bundle/config wiring, both unsigned Release builds, the fresh-user UI journey, the accessibility audit, the optimized Release contract, the launch packet, and known external setup warnings. `firebase:live-readiness` now requires every Rich V0 callable, including `syncOpenLARPCareerState`, to be deployed and active before it can report cloud readiness. As of August 12, 2026, that callable is absent from the development deployment and project billing is disabled; the private AI service is also absent. Billing authorization, deployment of the reviewed callable/private-service revision, and authenticated live Firebase/Gemini smoke therefore remain external service gates. The script also reports Firebase App Check App Attest registration and Firestore, Storage, and Google Identity enforcement status; warnings there mean App Check enforcement is still not beta-ready. A clean readiness script does not replace the required signed-in simulator/device smoke test for Google Sign-In, Firestore writes, Storage proof upload/read rules, and callable fallback behavior.
