...
- The workflow then runs Apple's accessibility audit without issue suppression across the core onboarding-to-first-proof path. It checks element descriptions, hit regions, contrast, detection, clipped text, traits, Dynamic Type, and other audit types exposed by the active XCTest runtime; manual VoiceOver and supported-device layout checks remain required.
