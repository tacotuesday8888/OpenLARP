...
- Repository-controlled product work now includes identity-safe persistence and erase, the complete 14-day sprint lifecycle, accessibility identifiers for the critical path, an automated fresh-user UI journey through onboarding, mission approval, all 14 persisted daily proof loops, both sprint reports, adaptive Chapter Two generation, and starting another sprint, plus Apple's accessibility audit across the core onboarding-to-first-proof path. Xcode 26.6's identified Dynamic Type false positive for the native onboarding-choice label is narrowly accepted only while a separate largest-accessibility-text UI scenario verifies its adaptive layout and usability.
...
- CI fails when simulator discovery fails, when either unsigned local-only or service-enabled generic Release build is absent, when the fresh-user UI journey, accessibility audit, or optimized contract test is skipped or does not run exactly once, or when the public Release app enables testability.
