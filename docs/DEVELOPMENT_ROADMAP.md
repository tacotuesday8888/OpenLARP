...
- Contextual `Ask OpenLARP` help across Cooked, mission, quest, proof preparation, proof feedback, and checkpoint reports, with ephemeral exchanges, confirmed-fact references, explicit inference/advice labels, deterministic fallbacks, and no memory writes or external actions
...
- A fail-closed Apple accessibility audit in CI across core onboarding, fact confirmation, Cooked evaluation, mission, quest, proof, feedback, and completion states, plus separate layout, contrast, and accessibility-hierarchy contracts for three narrowly reproduced Xcode 26.6 false positives: one native SwiftUI choice-label Dynamic Type report, one exact disabled primary-action text contrast report, and one target-outcome element-detection report with no attached element; manual VoiceOver and device-layout review still remain release checks
